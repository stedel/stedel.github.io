var app = (function () {
  ...
  // keep track of animation step state
  let animationStep = 0;

  // keep track, if the torus should rotate
  let torusRotationEnabled = true;

  // keep track, if the spheres should move
  ...
  /**
   * Init ea7 models, camera, webgl context, and key-event handler.
   */
  function start() {
    initWebGL();
    initShaderProgram();
    ...
    // about 60 fps
    setInterval(frameIntervalHandler, 16);
  }

  /**
   * Init WebGL context on canvas with id='ea7canvas' and set viewportWidth and -Height.
   */
  function initWebGL() {
    // Get canvas and WebGL context.
    const canvas = document.getElementById('ea7canvas');
  ...
  /**
   * Init vertex- and fragment shader with code in ea7.html
  ...
  function initModels() {
    ...
    createModel('torus', fs, colors.grey, [0, 0, 0], [0, 0, 0], [2, 2, 2], 0);
  ...
  /**
   * Update translation of all models, if movement is enabled for them.
   */
  function updateModelValues() {
    if (torusRotationEnabled) {
      const torus = models[1];
      torus.rotate[1] =
        torus.rotate[1] + Math.PI / (2 * animationFactors.torusRotation);
    }
    ...
    if (sphereMovementEnabled) {
      ...
      // prevent overflow
      animationStep = animationStep % animationFactors.sphereSteps;

      for (let i = 2; i < models.length; i++) {
        let animationOffset = 0.125 * (i - 2);
        models[i].translate[2] = Math.sin(
          (animationStep / animationFactors.sphereSteps + animationOffset) *
            (2 * Math.PI),
        );
      }
    }
  ...
  function initEventHandler() {
    ...
    window.onkeydown = function (evt) {
      ...
      // Toggle movement of models
      switch (c) {
        case 'J':
          // Toggle sphere movement
          sphereMovementEnabled = !sphereMovementEnabled;
          break;
        case 'K':
          // Toggle torus rotation
          torusRotationEnabled = !torusRotationEnabled;
      }
    };
  }

  /**
   * Update model values and then calls the render function.
   */
  function frameIntervalHandler() {
    updateModelValues();
    render();
  }
