const surfaceFunctions = {
  waveTorus: {
    x: (R, r, a, u, v) => {
      return (R + (r + a * Math.sin(5 * u)) * Math.cos(v)) * Math.cos(u);
    },
    y: (R, r, a, u, v) => {
      return (R + (r + a * Math.sin(5 * u)) * Math.cos(v)) * Math.sin(u);
    },
    z: (R, r, a, u, v) => {
      return -1 * (r + a * Math.sin(5 * u)) * Math.sin(v);
    },
  },
  turnedEightTorus: {
    x: (R, r, a, u, v) => {
    ...
    z: (R, r, a, u, v) => {
      return (
        r * (Math.sin(u / 2) * Math.sin(v) + Math.cos(u / 2) * Math.sin(2 * v))
      );
    },
  },
  cresent: {
    x: (a, b, c, u, v) => {
      return (
        (a + Math.sin(b * Math.PI * u) * Math.sin(b * Math.PI * v)) *
        Math.sin(c * Math.PI * v)
      );
    },
    y: (a, b, c, u, v) => {
      return (
        (a + Math.sin(b * Math.PI * u) * Math.sin(b * Math.PI * v)) *
        Math.cos(c * Math.PI * v)
      );
    },
    z: (a, b, c, u, v) => {
      return Math.cos(b * Math.PI * u) * Math.sin(b * Math.PI * v) + 4 * v - 2;
    },
  },
  custom: {
    x: (R, r, a, u, v) => {
    ...
    y: (R, r, a, u, v) => {
      return (R + (-r + a * Math.sin(5 * u)) * Math.cos(v)) * Math.sin(u * 2);
    },
    z: (R, r, a, u, v) => {
      return (r + a * Math.sin(5 * u)) * Math.sin(v);
    },
  },
};
...
function initCanvas(canvasId, params) {
  ...
  if (gl) {
    ...
    gl.bufferData(gl.ARRAY_BUFFER, resultData.vertices, gl.STATIC_DRAW);
